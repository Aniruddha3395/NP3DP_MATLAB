...
for II=1:8
if II==1
    [v, f, n, c, stltitle] = stlread('L1.stl');
elseif II==2
     [v, f, n, c, stltitle] = stlread('L2.stl');
elseif II==3
     [v, f, n, c, stltitle] = stlread('L3.stl');
elseif II==4
     [v, f, n, c, stltitle] = stlread('L4.stl');
elseif II==5
     [v, f, n, c, stltitle] = stlread('L5.stl');
elseif II==6
     [v, f, n, c, stltitle] = stlread('L6.stl');
elseif II==7
     [v, f, n, c, stltitle] = stlread('L7.stl');
else
...
% hold on;
% scatter3(bdry_val(1,1),bdry_val(1,2),bdry_val(1,3))
bdry_val = [x(a),y(a),z(a)];
fillpts = setdiff(fillpts,bdry_val,'rows');
Length =((bdry_val(:,1).^2)+(bdry_val(:,2).^2)).^(1/2); 
bdry1 = find(Length==min(Length));
bdry_val1 = [bdry_val(bdry1:size(bdry_val,1),:);bdry_val(1:bdry1,:)];
% figure;
% plot3(bdry_val1(:,1),bdry_val1(:,2),bdry_val1(:,3))
% hold on;
% scatter3(bdry_val1(1,1),bdry_val1(1,2),bdry_val1(1,3))
bdry2 = find(Length==max(Length));
bdry_val2 = [bdry_val(bdry2:size(bdry_val,1),:);bdry_val(1:bdry2,:)];
% figure;
% plot3(bdry_val2(:,1),bdry_val2(:,2),bdry_val2(:,3))
% hold on;
% scatter3(bdry_val2(1,1),bdry_val2(1,2),bdry_val2(1,3))
...
if (II/2)==round(II/2)
pathptsnew = [bdry_val2;flipud(pathptsnew(:,1:3))];
else
pathptsnew = [bdry_val1;pathptsnew(:,1:3)];
end    
Autopath = [Autopath;pathptsnew];

end

figure;
plot3(Autopath(:,1),Autopath(:,2),Autopath(:,3))
...
for i=1:size(Autopath,1)
px = Autopath(i,1); 
py = Autopath(i,2); 
pz = Autopath(i,3)+0.58;
rx = 0;
% ry = pathptsnew(i,5)/2; 
% rx = pathptsnew(i,4)/2;
